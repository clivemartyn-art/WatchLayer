...
if(!source||!output||resolve(source)===resolve(output))throw new Error('Usage: tsx scripts/replay-milestone9-live.ts saved-subset-directory new-output-directory');
...
await writeFile(join(output,'summary.json'),JSON.stringify({schemaVersion:1,mode:'M9 corrected-policy offline replay of the fresh eight-firm capture; no additional requests',source:resolve(source),benchmarkSha256:benchmark.sha256,rows},null,2)+'\n');
