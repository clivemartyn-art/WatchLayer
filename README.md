...
Fragments and common tracking parameters are removed and query parameters sorted. Root URLs normalize to `/`; non-root trailing slashes are preserved because servers may serve different content. Redirect aliases are remembered. Internal failures are requested once and reported with each known source page. Failures include HTTP errors and transport errors; a 403 or timeout does not prove a link is permanently broken.
