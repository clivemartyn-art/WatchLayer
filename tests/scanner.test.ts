...
describe('fixture scans', () => {
  ...
  it('deduplicates, avoids loops/external/doc downloads and records each broken source', async () => { const fetcher = fixture(); const r = await scan('example.com',{fetcher,pdfExtraction:false}); expect(r.summary.pagesScanned).toBe(2); expect(r.summary.pagesFailed).toBe(1); expect(r.brokenLinks).toHaveLength(2); expect(fetcher.mock.calls.filter(([url]) => url.endsWith('/missing'))).toHaveLength(1); expect(fetcher.mock.calls.some(([url]) => /outside|file.pdf/.test(url))).toBe(false); expect(r.documents[0].sourcePages).toEqual(['https://example.com/']); });
