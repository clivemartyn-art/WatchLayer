...
export async function runLawCli(args:string[]):Promise<void> {
  ...
  try{
    if(!args.length||args.includes('--help')){console.log('Usage: npm run lawwatch -- <scan-id or URL> [--db path] [--json] [--output file] [--reason-codes]');return;}
    const input=args[0];let db=DEFAULT_DATABASE,json=false,reasonCodes=false,output:string|undefined;
    for(let i=1;i<args.length;i++){const flag=args[i];if(flag==='--json')json=true;else if(flag==='--reason-codes')reasonCodes=true;else if(flag==='--db'||flag==='--output'){const value=args[++i];if(!value||value.startsWith('--'))throw new Error(`${flag} requires a value`);if(flag==='--db')db=value;else output=value;}else throw new Error(`Unknown option: ${flag}`);}
    ...
    if(output)await writeFile(output,serialized+'\n');console.log(json?serialized:lawWatchReport(report,reasonCodes));
