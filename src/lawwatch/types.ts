...
export interface PageFacts {url:string; title:string; contentHash:string; reliable:boolean; services:ServiceMatch[]; signals:Record<string,Fact[]>; links:SourceLink[]; pricing:boolean; complaints:boolean; quote_generator_detected:boolean; excludedContent?:boolean; serviceSignals?:Partial<Record<Service,Record<string,Fact[]>>>}
export interface FactSet {schemaVersion:1; detectorVersion:'1.0'|'1.1'; scanId:string; pages:PageFacts[]}
...
export interface LawReport {schemaVersion:1; site:string; scanId:string; packId:'lawwatch-england-wales'; packVersion:'1.0'|'1.1'; universalResults:Result[]; classifications:ServiceClassification[]; inventory:Surface[]; results:LawResult[]; changes:LawResult[]; drift:LawResult[]; summary:Record<State,number>; runs:RuleRun[]; statement:string}
