...
export function extractPage(response: Response): Page {
  const $ = load(response.body); const text = visibleText(response.body); const wordCount = text ? text.split(/\s+/).length : 0;
