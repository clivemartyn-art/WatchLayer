...
export const USER_AGENT = 'WatchLayer/0.1 (local website scanner)';
    ...
    for (let hop = 0; hop <= 5; hop++) {
      ...
      const response = await new Promise<{status: number; contentType: string; location?: string; body: string}>((resolve, reject) => {
        ...
        const req = (url.protocol === 'https:' ? https : http).request(url, {
          method: 'GET', agent: false, headers: { 'user-agent': USER_AGENT, accept: 'text/html,application/xml,text/plain;q=0.9', 'accept-encoding': 'identity' },
          ...
          if (status >= 300 && status < 400 || !/text\/|xml|html/i.test(contentType)) { resolve({status, contentType, location: res.headers.location, body: ''}); res.destroy(); return; }
